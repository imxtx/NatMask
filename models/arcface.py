...
class IResNet(nn.Module):
    ...
    def forward(self, x):
        with torch.amp.autocast(device_type=x.device.type, dtype=torch.float):
            x = self.conv1(x)
            x = self.bn1(x)
            x = self.prelu(x)
            x = self.layer1(x)
...
def arcface(device="cpu"):
    """Create an ArcFace model pre-trained on MS1MV3(MS1M-RetinaFace) dataset.
    ...
    """
    logging.info("Loading pre-trained ArcFace model ...")
    import pathlib

    cur_dir = pathlib.Path(__file__).parent.parent.resolve()
    model = iresnet50()
    state_dict = torch.load(
        f"{str(cur_dir)}/pretrained_models/backbone_ires50_ms1mv3.pth",
        map_location=device,
    )
