...
def load_weights(mdl, name):
    ...
    state_dict = torch.load(
        f"{str(root_dir)}/pretrained_models/20180402-114759-vggface2.pth"
    )

    mdl.load_state_dict(state_dict)


def facenet(device="cpu"):
    """Create a FaceNet model pre-trained on VggFace2

    Input size: any, 160x160 is recommended
    FaceNet paper: https://arxiv.org/abs/1503.03832
    FaceNet implementation: https://github.com/timesler/facenet-pytorch
    """
    logging.info("Loading pre-trained FaceNet model ...")

    model = InceptionResnetV1(pretrained="vggface2", device=device)
    # For a model pretrained on CASIA-Webface, LFW Accuracy: 99.05%
