...
def map_texture(
    ...
    for y in range(h):
        for x in range(w):
            ...
            # src
            src_texel = (
                w0 * src_vertices[:2, tri[0]]
                + w1 * src_vertices[:2, tri[1]]
                + w2 * src_vertices[:2, tri[2]]
            )  #
            #
            if (
                src_texel[0] < 0
            ...
            if mapping_type == "nearest":
                dst_image[y, x, :] = src_image[
                    int(round(src_texel[1])), int(round(src_texel[0])), :
                ]
            # bilinear
            elif mapping_type == "bilinear":
                # next 4 pixels
                ul = src_image[
                    int(np.floor(src_texel[1])), int(np.floor(src_texel[0])), :
                ]
                ur = src_image[
                    int(np.floor(src_texel[1])), int(np.ceil(src_texel[0])), :
                ]
                dl = src_image[
                    int(np.ceil(src_texel[1])), int(np.floor(src_texel[0])), :
                ]
                dr = src_image[
                    int(np.ceil(src_texel[1])), int(np.ceil(src_texel[0])), :
                ]
